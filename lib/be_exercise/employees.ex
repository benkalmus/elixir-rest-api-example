defmodule Exercise.Employees do
  ...
  def batch_write(employee_attrs) do
    {created_employees, invalid_changesets} =
      generic_batch_write(employee_attrs, &create_employee/1)
  ...
  def salary_metrics_by_job_title(job_title, target_currency \\ "USD") do
    query =
      from e in Employee,
        join: country in Exercise.Countries.Country,
        on: country.id == e.country_id,
        join: currency in Exercise.Countries.Currency,
      ...
      transaction_result =
        Repo.transaction(fn ->
          Repo.insert_all(Employee, chunk,
            # insert_all options:
            # replace placeholders with datetime
  ...
  defp handle_salary_metrics_by_country(%{count: num} = query_result, country_id) when num > 0 do
    # todo, find out if we can retrieve currency_code in one query
    currency_code =
  ...
  defp handle_salary_metrics_by_country(query_result, _) do
    Logger.error(
      "Salary metrics by country query failed to produce metrics in #{inspect(query_result)}"
    )
